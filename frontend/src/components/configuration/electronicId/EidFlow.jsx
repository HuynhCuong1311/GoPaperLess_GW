...
export const EidFlow = ({
  ...
  const EidCertificate = useGetEidCertificate();
  ...
  const handleSubmitClick = () => {
    switch (state.activeStep) {
        ...
        updateIdentitySubject.mutate(
          ...
          },
          {
            onSuccess: (data) => {
              ...
              dispatch({ type: "SET_PROCESS_ID", payload: data });
              dispatch({ type: "SET_DISABLED", payload: true });
              dispatch({ type: "SET_ACTIVE_STEP", payload: 11 });
            },
            onError: (error) => {
              console.log("error: ", error);
              dispatch({
      ...
      case 14:
        ...
        //         jwt: state.jwt,
        //         assurance:
        //           state.data.assurance === "NORMAL" ||
        //           state.data.assurance === "QES"
        //             ? "NORMAL"
        //             : "SEAL",
  ...
  const steps = [
    <Suspense key={1} fallback={<div>Loading...</div>}>
      <SmartIdCriteria
        state={state}
        dispatch={dispatch}
        criteriaList={filterPrefix?.filter((item) => item.type !== "PHONE-ID")}
