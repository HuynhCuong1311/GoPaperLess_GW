...
const SelectProviderForCreate = lazy(() =>
  import("@/components/configuration/components/SelectProviderForCreate")
);
...
const dataFetchReducer = (state, action) => {
  switch (action.type) {
    ...
    case "SET_REQUEST_ID":
      return {
        ...state,
        data: { ...state.data, requestId: action.payload },
      };
    case "SET_ASSURANCE":
      return {
        ...state,
    ...
    case "SET_CODE_ENABLE":
      return {
        ...state,
        data: { ...state.data, codeEnable: action.payload },
      };
    case "SET_ACCESS_TOKEN":
      return {
        ...state,
        data: { ...state.data, accessToken: action.payload },
      };
    case "SET_REMOTE_SIGNING_ACCESS_TOKEN":
      return {
        ...state,
        data: { ...state.data, remoteSigningAccessToken: action.payload },
      };
    ...
    switch (state.activeStep) {
      case 1:
        switch (state.data.connectorName) {
          ...
          case "SMART_ID_LCA":
            {
              let codeNumber = state.data.code;
              if (state.data.criteria === "PHONE") {
                let phoneWithoutDialCode = state.data.code.slice(
                  state.dialCode.length
                );
                if (phoneWithoutDialCode.match(/^0+/)) {
                  // Remove all leading '0's, leaving at least one '0'
                  phoneWithoutDialCode = phoneWithoutDialCode.replace(
                    /^0+/,
                    ""
                  );
                  // dispatch({
                ...
                codeNumber = state.dialCode + phoneWithoutDialCode;
              }
              smartIdCertificate.mutate(
                {
                  language: lang,
                  codeNumber: state.data.criteria + ":" + codeNumber,
                  connectorName: state.data.connectorName,
                },
                {
                  onSuccess: (data) => {
                    dispatch({
                      type: "SET_CERT_LIST",
                      payload: data.listCertificate,
                    });
                    dispatch({
                      type: "SET_CODE_ENABLE",
                      payload: data.codeEnable,
                    });
            ...
            misaCertificate.mutate(
              ...
              },
              {
                onSuccess: (data) => {
                  ...
                  dispatch({
                    type: "SET_REMOTE_SIGNING_ACCESS_TOKEN",
                    payload: data.login.data.remoteSigningAccessToken,
                  });
                  dispatch({ type: "SET_ACTIVE_STEP", payload: 2 });
                  // setActiveStep(2);
                },
      ...
      case 3:
        ...
        if (!state.data.certSelected) {
          if (state.data.connectorName === "SMART_ID_MOBILE_ID") {
          ...
          if (state.data.connectorName === "SMART_ID_MOBILE_ID") {
            ...
            handleSubmitModel({ ...state.data, requestID });
          } else {
